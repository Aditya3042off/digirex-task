...
const registerUser = asyncHandler(async(req,res,next) => {
    ...
    //creating the user
    const newUser = new User({name,email,password:hashedPassword,recordList:[]});
    await newUser.save();
...
const getReports = asyncHandler(async(req,res,next) => {
    ...
    //finding user
    const user = await User.findById(id).select("-password").populate("recordList");

    const {name,email,recordList,} = user;
    //sending this information to front end
    res.status(200).json({id,name,email,recordList});
})

module.exports = {authUser,registerUser,getReports};
